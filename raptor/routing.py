...
class HttpMethodsMap():
  """
  Class for storing an retrieving function-pointers corresponding to a specific
  HTTP Method.
  """
  ...
  def register(self, http_methods: list[str], func: FunctionType) -> None:
    ...
    Args:
      ...
      func (FunctionType): Function-pointer to function, that should be executed
        when http-methods are called.
...
class Router():
  ...
  def mount(self, route_string: str, func: FunctionType,
    ...
    Args:
      ...
      accepted_http_methods (list[str]): List of HTTP Methods that are accepted.
    """
